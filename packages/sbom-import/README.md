...
> **This package is not on npm — deliberately.** `@capwall/policy-schema`, `@capwall/core` and
> `@capwall/cli` are published; `@capwall/sbom-import` is **excluded from the publish set on
> purpose**: nothing consumes it, so it would land on the registry as an unreachable library. It
> stays in version lockstep in-repo. The publish set is declared once, in
> `scripts/check-release-versions.mjs` (`--publish-list`) — see
